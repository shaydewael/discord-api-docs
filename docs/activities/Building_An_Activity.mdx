...
When requesting scopes, you'll need your app's OAuth2 identifiers. In your app's settings click on **OAuth2** on the sidebar and copy the values:
- **Client ID** is the public ID that Discord associates with your app, and is almost always the same as your App ID.
- **Client Secret** is a private identifier that your app will use to grant an OAuth2 `access_token`. It should never be stored in the client-side JavaScript code.

Copy and store both of these values somewhere safe so we can use them later on in our code.
