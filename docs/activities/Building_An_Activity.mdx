...
Before continuing with Step 2, make sure you have the following:
1. Understand what [Activities](#DOCS_ACTIVITES_OVERVIEW) are (TODO)
2. Have a local version of the [sample project](https://github.com/discord/getting-started-activity) with its frontend dependencies installed.
</Collapsible>
...
Next, we'll add a redirect URI, which is where a user is redirected to after authorization with your app. The Embedded App SDK handles redirecting a user back to your Activity when [authorizing](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/authorize), so the value for the redirect URI isn't important right now unless you want to offer authorization outside of your Activity. TODO: can tie more to oauth
...
We'll need to add two values to this `.env` file for now. Back in your app's settings, click on **OAuth2** on the sidebar:
...
TODO: new icon for this collapse, maybe collapse by default? or maybe in a callout with new style

<Collapsible title="Step 2 Checkpoint" icon="list" open>

By the end of Step 2, make sure you have:
1. A Discord app created with Activities enabled and a placeholder Redirect URI added
2. Your Discord app's Client ID and Client Secret added to your project's `.env` file.
</Collapsible>

## Step 3: Setting Up the Embedded App SDK

With our project and Discord app set up, we're going to install and configure the [Embedded App SDK](#TODO). The Embedded App SDK handles the communication between Discord and your app, and we'll use it extensively through the rest of the guide.
...
We also set up our check for the [`ready` event](#TODO) with an async/await function. This allows us to output a log or take action once the initial handshake was successful.

TODO: explain events/RPC maybe
...
By the end of Step 3, make sure you should:
1. Have the Embedded App SDK installed to your project
2. Imported the SDK in your project's `client/main.js` file
</Collapsible>
...
Let's run the sample app. Open a terminal window and navigate to your project directory's `client` folder, then start the client-side app:
...
We'll use the **Local** URL as a publicly-accessible URL in the next step.

### Set up a public endpoint

Next, we'll need to set up public endpoint to TODO: x, y. To do that, we'll create a tunnel using a reverse proxy. While we'll be using cloudflared in this guide, you can use ngrok or another reverse proxy solution if you prefer.

While your app is still running, we'll start a network tunnel that listens to the port from the last step (in this case, port `5173`).
...
To serve your application, you need to add a public URL mapping to where your app is hosted. TODO: why? this lets you do x, y. Since we're developing locally, we'll use the network tunnel we started above. TODO: move this to above maybe? idk

In your [app's settings](#TODO), click on the **URL Mappings** page under **Activities** on the left-hand sidebar. Enter the URL you generated from `cloudflared` in the previous step.
...
In a voice channel in your Discord server, click on the Activities icon (🚀) to see your Activity at the top of the Activity Shelf. TODO: not voice channel
...
By the end of Step 4, make sure you should:
1. TODO
</Collapsible>
...
By the end of Step 5, make sure you should:
1. TODO
</Collapsible>
