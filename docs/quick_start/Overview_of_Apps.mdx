...
Apps can use the [Embedded App SDK](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK) to build **[Activities](#DOCS_ACTIVITIES_OVERVIEW)** which are social, multiplayer games and  experiences in Discord. Activities are cross-platform and embedded within iframes in Discord, where friends who play games are already hanging out.
...
This developer documentation is full of nooks and crannies with all sorts of features and functionality to explore. Discover the possibilities by [building your own app](https://discord.com/developers/applications).
...
The **RPC API** allows you to send [commands](#DOCS_TOPICS_RPC/commands-and-events-rpc-commands) and receive [events](#DOCS_TOPICS_RPC/commands-and-events-rpc-events) to interact with authenticated Discord clients. Accessing the RPC API directly is in closed beta, but the [Embedded App SDK](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK) uses the RPC API to allow [Activities](#DOCS_ACTIVITIES_OVERVIEW) to interact with a user's client.

Use the RPC API to:
- Retrieve data from a user's Discord client
- Listen to events related to state changes in a user's Discord client
